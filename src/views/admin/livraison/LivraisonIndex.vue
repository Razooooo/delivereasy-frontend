    ...
    <table>
      ...
      <tbody>
        <tr v-for="livraison in livraisons" :key="livraison.idLivraison">
          ...
          <td>{{ livraison.idLivreur || 'Non assigné' }}</td>
          <td>
    ...
    <div v-if="selectedLivraison" class="edit-form">
      ...
      <form @submit.prevent="updateLivraison">
        ...
        <div>
          ...
          <select id="statut" v-model="selectedLivraison.statut" required>
        ...
        <div>
          <label for="livreur">ID Livreur:</label>
          <input type="number" id="livreur" v-model.number="selectedLivraison.idLivreur" placeholder="Optionnel" />
        </div>
    ...
    <div class="form-add-livraison">
      ...
      <form @submit.prevent="addLivraison">
        ...
        <div>
          ...
          <select id="new-statut" v-model="newLivraison.statut" required>
            <option value="en cours">En cours</option>
            <option value="terminée">Terminée</option>
            <option value="annulée">Annulée</option>
          </select>
        ...
        <div>
          <label for="new-livreur">ID Livreur:</label>
          <input type="number" id="new-livreur" v-model.number="newLivraison.idLivreur" placeholder="Optionnel" />
        </div>
...
interface Livraison {
  idLivraison: number;
    ...
    statut: "en cours" | "terminée" | "annulée" | string;
...
export default defineComponent({
  ...
  async updateLivraison() {
    ...
    try {
      const { idLivraison, Date_livraison, statut, idLivreur } = this.selectedLivraison;
  ...
  async deleteLivraison(id: number) {
    console.log('ID de livraison à supprimer:', id);
    if (!id) {
